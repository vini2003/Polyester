package com.github.vini2003.polyester.serializer;

import com.github.vini2003.polyester.Polyester;
import com.github.vini2003.polyester.utility.IdentifierUtilities;
...
public class ItemStackSerializer {
	...
	public static CompoundTag toTag(ItemStack stack, CompoundTag tag) {
		...
		if (stack.getCount() < 0) {
			Polyester.LOGGER.log(Level.ERROR, "ItemStack failed to be written: count was smaller than zero!");
	...
	public static ItemStack fromTag(CompoundTag tag) {
		...
		if (!IdentifierUtilities.isValid(identifierString)) {
			Polyester.LOGGER.log(Level.ERROR, "ItemStack failed to be read: " + CompoundTag.class.getName() + "'s 'id' value is not a valid " + Identifier.class.getName() + "!");
			return ItemStack.EMPTY;
		}
		...
		if (!Registry.ITEM.getIds().contains(identifier)) {
			Polyester.LOGGER.log(Level.ERROR, "ItemStack failed to be read: item registry did not contain a valid item identifier!");
			return ItemStack.EMPTY;
		}

		if (!tag.contains("Count")) {
			Polyester.LOGGER.log(Level.ERROR, "ItemStack failed to be read: " + CompoundTag.class.getName() + " does not contain 'Count' value!");
			return ItemStack.EMPTY;
		}
		...
		if (count < 0) {
			Polyester.LOGGER.log(Level.ERROR, "ItemStack failed to be read: count was smaller than zero!");
			return ItemStack.EMPTY;
		}
		...
		if (tag.contains("tag", 10)) {
			...
			if (!(rawTag instanceof CompoundTag)) {
				Polyester.LOGGER.log(Level.WARN, "ItemStack did not fail to be read, but had a non-standard tag which was discarded!");
			} else {
