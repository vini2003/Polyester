package com.github.vini2003.polyester.api.inventory;

import com.github.vini2003.polyester.Polyester;
import com.github.vini2003.polyester.api.data.Range;
...
/**
 * An inventory component is an
 * inventory implementation focusing
 * on performance and ease of use.
 */
public interface InventoryComponent {
	/**
	 * Instantiates an InventoryComponent
	 * wrapper over a default Inventory.
	 */
	static InventoryComponentFromInventory of(Inventory inventory) {
		return InventoryComponentFromInventory.of(inventory);
	}
	...
	default void read(InventoryComponent target, CompoundTag tag, Optional<String> subtag, Optional<Range<Integer>> range) {
		...
		if (!(rawTag instanceof CompoundTag)) {
			Polyester.LOGGER.log(Level.ERROR, "Inventory contents failed to be read: " + rawTag.getClass().getName() + " is not instance of " + CompoundTag.class.getName() + "!");
			return;
		}
		...
		if (!compoundTag.contains("size")) {
			Polyester.LOGGER.log(Level.ERROR, "Inventory contents failed to be read: " + CompoundTag.class.getName() + " does not contain 'size' value!");
			return;
		}
		...
		if (size == 0) {
			Polyester.LOGGER.log(Level.WARN, "Inventory contents size successfully read, but with size of zero. This may indicate a non-integer 'size' value!");
		}

		if (!compoundTag.contains("stacks")) {
			Polyester.LOGGER.log(Level.ERROR, "Inventory contents failed to be read: " + CompoundTag.class.getName() + " does not contain 'stacks' subtag!");
			return;
		}
		...
		if (!(rawStacksTag instanceof CompoundTag)) {
			Polyester.LOGGER.log(Level.ERROR, "Inventory contents failed to be read: " + rawStacksTag.getClass().getName() + " is not instance of " + CompoundTag.class.getName() + "!");
			return;
		}
		...
		if (size < maximum) {
			Polyester.LOGGER.log(Level.WARN, "Inventory size from tag smaller than specified maximum: will continue reading!");
			maximum = size;
		}

		if (target.getSize() < maximum) {
			Polyester.LOGGER.log(Level.WARN, "Inventory size from target smaller than specified maximum: will continue reading!");
		...
		for (int position = minimum; position < maximum; ++position) {
			if (stacksTag.contains(String.valueOf(position))) {
				...
				if (!(rawStackTag instanceof CompoundTag)) {
					Polyester.LOGGER.log(Level.ERROR, "Inventory stack skipped: stored tag not instance of " + CompoundTag.class.getName() + "!");
					return;
				}
	...
	default void dispatchConsumers() {
		getListeners().forEach(consumer -> consumer.accept(this));
	}
